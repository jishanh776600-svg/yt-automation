...
class TopicDiscoveryEngine:
    ...
    def is_duplicate(self, db: Session, title: str) -> bool:
        ...
        for t in existing:
            if clean_title in t.title.lower() or t.title.lower() in clean_title:
                return True
        return False
    ...
    def discover_topics(self, db: Session, limit: int = 5) -> List[Topic]:
        ...
        # 1. First check if we have approved topics in DB that have not been published yet
        from core.models import Job, UploadRecord
        published_topic_ids = {j.topic_id for j in db.query(Job).filter(Job.state == "PUBLISHED").all() if j.topic_id}
