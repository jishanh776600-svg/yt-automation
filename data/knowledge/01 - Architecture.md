...
> **Scope:** Multi-tier architectural topology, storage segregation, distributed cloud locking, and concurrency boundaries `[CODE VERIFIED]`.  
> **Master Reference:** [[02 — ARCHITECTURE/Cloud & Storage Topology|Cloud & Storage Topology]]
...
|   - Installs FFmpeg, fonts, Python 3.11, and pip dependencies.                                    |
...
|   - Human-readable Markdown knowledge records organized in canonical 10-tier hierarchy.           |
+---------------------------------------------------------------------------------------------------+
```
...
2. **Hardened Invariants (Commit `31c002c`):**
   - TTL reduced to **900s (15 minutes)**.
   - Background heartbeat thread refreshes lock every **120s**.
   - Dead-runner reclamation via GitHub Actions API.
   - CLI flag `--force-unlock` and workflow dispatch parameter `force_unlock: true`.
3. **Dedicated Lock Names:** `production`, `publisher`, `analytics`.
...
|---|---|---|---|
| `.github/workflows/produce_buffer.yml` | `0 */3 * * *` (Every 3 Hours) | `python main.py --maintain-buffer 6` | Audits `01_READY`, computes deficit, produces replacement Shorts sequentially to restore stock to 6. |
